...
DOCS_PAGE="${REPO_ROOT}/docs/docs/components/${NAME_LOWER}.mdx"
...
# ── Docs page ─────────────────────────────────────────────────────────────────
# Docusaurus's sidebar is autogenerated from the docs/docs/components/ folder
# (see sidebars.ts) — a correctly-named, kebab-case .mdx file dropped in here
# is sufficient for correct sidebar placement, no separate sidebar file to
# patch (unlike the old Astro site's hand-curated array).
cat > "$DOCS_PAGE" << EOF
---
title: ${NAME_PASCAL}
description: TODO: describe the ${NAME_PASCAL} component
slug: /components/${NAME_LOWER}
---
...
TODO: describe the ${NAME_PASCAL} component.
...
<FrameworkTabs
  ...
  react={\`
import { ${NAME_PASCAL} } from '@latty-ds/react';
...
<ApiTable tag="${TAG}" />
EOF
echo "✅  Created docs/docs/components/${NAME_LOWER}.mdx"

# ── components.ts (docs gallery data) ─────────────────────────────────────────
COMPONENTS_DATA="${REPO_ROOT}/docs/src/data/components.ts"

if grep -qF "name: '${NAME_PASCAL}'" "$COMPONENTS_DATA"; then
  echo "ℹ️   components.ts entry already present"
else
...
JSEOF
  echo "✅  Added ${NAME_PASCAL} to docs/src/data/components.ts"
fi

# ── Build @latty-ds/web ────────────────────────────────────────────────────────
echo ""
echo "Building @latty-ds/web to register ${NAME_PASCAL} in dist and manifest..."
pnpm --filter @latty-ds/web build
echo "✅  @latty-ds/web built"
