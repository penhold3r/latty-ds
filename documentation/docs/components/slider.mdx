---
title: Slider
description: A range input for selecting a numeric value within a bounded range
...
and exposes <code>input</code> (fires continuously while dragging) and <code>change</code> (fires once on release) events,
both carrying the current numeric value in <code>event.detail.value</code>.

## Playground

<ComponentPlayground tag="lt-slider" stretch={true} previewHeight="auto" defaults={{ label: 'Volume', value: 40 }} />
