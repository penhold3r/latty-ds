...
- **Monorepo Structure:** pnpm workspace.
  ...
  - `docs`: Docusaurus/MDX documentation site.
- **Naming Conventions:**
  - npm scope: `@latty-ds/*`
...
- **Documentation:**
  - The docs site must showcase the system. Use Latty components (`lt-text`, `lt-button`, etc.) in documentation pages instead of plain HTML where possible — this includes Docusaurus's own chrome (headings, breadcrumbs, admonitions, pagination, footer), swizzled under `docs/src/theme/`.
  - Component pages are `.mdx` files under `docs/docs/components/`; the sidebar is autogenerated, no separate nav file to edit.
