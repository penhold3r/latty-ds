...
navigation (Arrow, Enter, Escape) is fully supported. Use <code>lt-select</code> when no filtering is needed.

## Playground

<ComponentPlayground
  tag="lt-combobox"
  defaults={{ label: 'Country', placeholder: 'Search countries…' }}
  seedData={{
    options: [
