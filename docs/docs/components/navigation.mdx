---
title: Navigation
description: Horizontal and vertical nav with nested collapsible items
...
<code>&lt;lt-nav-item&gt;</code> elements to create collapsible groups — in vertical mode they expand inline; in
...
### SPA router integration
  ...
  <code>nav-item-click</code> is cancelable: call <code>preventDefault()</code> to stop the anchor's full-page navigation
    and hand the URL to your router. Drive the highlighted item with the <code>active</code> property — it also sets <code>aria-current="page"</code> on the internal trigger.
  <FrameworkTabs
